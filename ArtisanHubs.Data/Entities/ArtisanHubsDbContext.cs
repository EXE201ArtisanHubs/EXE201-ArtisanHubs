...
public partial class ArtisanHubsDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        // Cấu hình cho FavoriteProduct (đã thêm)
        modelBuilder.Entity<FavoriteProduct>(entity =>
        {
            // Thiết lập khóa chính composite
            entity.HasKey(fp => new { fp.AccountId, fp.ProductId });

            // Cấu hình mối quan hệ với Account
            entity.HasOne(fp => fp.Account)
                .WithMany(a => a.FavoriteProducts)
                .HasForeignKey(fp => fp.AccountId)
                .OnDelete(DeleteBehavior.ClientSetNull) // Quan trọng: Tránh xóa cascade
                .HasConstraintName("favoriteproduct_account_id_fkey"); // Đặt tên cho ràng buộc FK
            ...
            entity.HasOne(fp => fp.Product)
                ...
                .OnDelete(DeleteBehavior.ClientSetNull) // Quan trọng: Tránh xóa cascade
                .HasConstraintName("favoriteproduct_product_id_fkey"); // Đặt tên cho ràng buộc FK
        });
